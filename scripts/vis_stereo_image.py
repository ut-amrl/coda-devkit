...
#     closest_img_path = join(img_dir, img_file)
#     img_np = cv2.imread(closest_img_path)
#     max_val = np.max(img_np)
#     rescale_img_np = img_np * (255 / max_val)
#     # import pdb; pdb.set_trace()
#     outpath = join(out_dir, img_file)
#     cv2.imwrite(outpath, rescale_img_np)
#     import pdb; pdb.set_trace()

# target_ts = 1675698089.964643

# print("Searching in %s for timestamp %s" % (img_dir, str(target_ts)))
# closest_img_idx = np.argmin(np.abs(target_ts - img_ts))
# closest_img_path = join(img_dir, img_files[closest_img_idx])
# print("Closest image found with path %s "%closest_img_path)
...
# rescale_img_np = img_np * (255 / max_val)
# # import pdb; pdb.set_trace()
# cv2.imwrite("test.png", rescale_img_np)